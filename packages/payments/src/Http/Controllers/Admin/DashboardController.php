...
class DashboardController extends ParentDashboardController
{
    ...
    protected function getAccountingBalance(Request $request): Collection
    {
        $getData = function () {
            return collect([
                (float) Deposit::completed()->where(function (Builder $query) { Filter::byPeriod($query); })->sum('amount'),
                (float) Withdrawal::completed()->where(function (Builder $query) { Filter::byPeriod($query); })->sum('amount')
            ]);
        };
