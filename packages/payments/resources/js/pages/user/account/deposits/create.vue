...
export default {
    ...
    paymentAmount () {
      ...
      return this.rate > 0 && this.form.amount > 0
        ? decimal(
          ...
          decimalPlaces || (['coinpayments', 'ethereum'].indexOf(gatewayCode) > -1 ? 8 : 2)
        )
        : 0
    },
    paymentCurrencies () {
  ...
  watch: {
    method (method) {
      ...
      axios.defaults.withCredentials = get(method, 'payment_method.gateway.code') !== 'tron'
      ...
      if (get(method, 'payment_method.gateway.code') === 'stripe') {
        if (this.stripe.instance) {
          this.$nextTick(() => { this.initStripe() })
        } else {
          loadScript('https://js.stripe.com/v3/', this.initStripe)
        }
      } else if (get(method, 'payment_method.gateway.code') === 'ethereum') {
        this.initWeb3()
      } else if (get(method, 'payment_method.gateway.code') === 'tron') {
        this.initTronWeb()
      }

      this.form.payment_currency = this.paymentCurrencies ? Object.keys(this.paymentCurrencies)[0] : null
    },
    isEthereumAddressConfirmed: function (confirmed) {
      if (confirmed) {
    ...
    isTronAddressConfirmed: function (confirmed) {
      if (confirmed) {
        this.form.errors.clear('parameters.address')
      } else if (!confirmed && this.form.parameters.address) {
        this.form.errors.set('parameters.address', this.$t('Please confirm this address belongs to you by signing a random text string.'))
  ...
  methods: {
    ...
    async initTronWeb () {
      ...
      if (address !== false) {
        ...
        this.fetchConfirmedTronAddresses()
      } else {
        this.$store.dispatch('message/error', { text: this.$t('Access to your account is not authorized') })
        this.formIsValid = false
      }
    },
    async fetchConfirmedEthereumAddresses () {
      if (!this.ethereum.confirmedAddresses) {
        const { data } = await axios.get('/api/ethereum/addresses')

        this.ethereum.confirmedAddresses = data
      }
    },
    async fetchConfirmedTronAddresses () {
      if (!this.tron.confirmedAddresses) {
        const { data } = await axios.get('/api/tron/addresses')

        this.tron.confirmedAddresses = data
      }
    },
    async confirmEthereumAddress () {
      this.ethereum.addressConfirmationInProgress = true

      const { data } = await axios.post('/api/ethereum/addresses', { address: this.form.parameters.address })

      let signature
      const address = data
      const web = this.ethereum.web
      ...
      if (signature) {
        const request = await axios.post(`/api/ethereum/addresses/${address.id}/verify`, { signature })

        this.ethereum.confirmedAddresses.push(request.data)
      }

      this.ethereum.addressConfirmationInProgress = false
    },
    async confirmTronAddress () {
      this.tron.addressConfirmationInProgress = true

      const { data } = await axios.post('/api/tron/addresses', { address: this.form.parameters.address })
      ...
      if (signature) {
        const { data } = await axios.post(`/api/tron/addresses/${address.id}/verify`, { signature })
