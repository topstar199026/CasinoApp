          ...
          <v-card-text>
            ...
            <v-row v-if="withdrawal && withdrawal.is_created" class="mt-5">
              <v-col class="text-center">
                ...
                <v-tooltip v-if="get(withdrawal, 'method.payment_method.gateway.code') === 'ethereum'" bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn
                      color="primary"
...
export default {
  ...
  computed: {
    ...
    ethereumTransactionCompleted () {
      return get(this.withdrawal, 'method.payment_method.gateway.code') === 'ethereum'
        && this.withdrawal
  ...
  methods: {
    ...
    async createEthereumTransaction () {
      ...
      if (addresses) {
        try {
          // ERC20 token
          if (this.withdrawal.method.payment_method.code === 'erc20') {
